#!/usr/bin/python
# CAUTION! reference sequence name should not used spaces
#USAGE: ./3.sam2fasta.py fastaReference purifiedSamFile fastaMultipleAlignment

# written by Chang Park (naltang@yahoo.com) 2016.8.23
...
def	read_fasta_file (file_in, callback):
	...
	for line in f:
		...
		if (line.startswith (">")):
			if (seq != ""):
				callback (name, seq)
			name = line[1:].split()[0]  # here was added spliting
			seq = ""
		else:
			line = line.replace (" ", "")
			line = line.upper ()
